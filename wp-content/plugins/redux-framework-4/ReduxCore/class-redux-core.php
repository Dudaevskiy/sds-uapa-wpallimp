...
if ( ! class_exists( 'Redux_Core', false ) ) {
	...
	class Redux_Core {
		...
		private function includes() {
			...
			add_action( 'wp_ajax_nopriv_' . $support_hash, [ 'Redux_Helpers', 'support_args' ] );
			add_action( 'wp_ajax_' . $support_hash, [ 'Redux_Helpers', 'support_args' ] );
			$hash_arg = md5( trailingslashit( network_site_url() ) . '-redux' );
			add_action( 'wp_ajax_nopriv_' . $hash_arg, [ 'Redux_Helpers', 'hash_arg' ] );
			add_action( 'wp_ajax_' . $hash_arg, [ 'Redux_Helpers', 'hash_arg' ] );
			add_action( 'wp_ajax_redux_support_hash', [ 'Redux_Functions', 'support_hash' ] );

			add_filter( 'redux/tracking/options', [ 'Redux_Helpers', 'redux_stats_additions' ] );
		}
