...
class Redux_Rest_Api_Builder {
	...
	public function get_field( $request = [] ) {
		...
		if ( isset( $field_classes[ strtolower( $type ) ] ) ) {
			...
			$field_class = [ 'Redux_' . ucwords( $type ), 'ReduxFramework_' . ucwords( $type ) ];
	...
	public function render_field( $request = [] ) {
		...
		if ( isset( $field_classes[ strtolower( $type ) ] ) ) {
			...
			if ( $field_class && is_subclass_of( $field_class, 'Redux_Field' ) ) {
				 ...
				 * @var Redux_Descriptor $descriptor
				 */
				$descriptor = call_user_func( [ 'ReduxFramework_' . $type, 'get_descriptor' ] );
				$opt_name   = 'redux_builder_api';

				$redux_instance = new ReduxFramework( [], [ 'opt_name' => $opt_name ] );
