...
if ( ! class_exists( 'Redux_WordPress_Data', false ) ) {
	...
	class Redux_WordPress_Data extends Redux_Class {
		 ...
		 */
		public function get( $type = false, $args = [], $current_value = '' ) {
			...
			if ( ! empty( $current_value ) && ( is_array( $current_value ) || Redux_Helpers::is_integer( $current_value ) ) ) {
				switch ( $type ) {
					...
					case 'page':
						...
						$pages = get_pages( [ 'include' => $current_value ] );
						if ( ! empty( $pages ) ) {
					...
					case 'category':
						...
						$terms = get_categories( [ 'object_ids' => $current_value ] );
						if ( ! empty( $terms ) ) {
					...
					case 'tag':
						...
						$terms = get_tags( [ 'object_ids' => $current_value ] );
						if ( ! empty( $terms ) ) {
							foreach ( $terms as $term ) {
								$current_data[ $term->term_id ] = $term->name;
							}
					...
					case 'menu':
						...
						$menus = wp_get_nav_menus( [ 'object_ids' => $current_value ] );
						if ( ! empty( $menus ) ) {
					...
					case 'posts':
						...
						$posts = get_posts( [ 'post__in' => $current_value ] );
						if ( ! empty( $posts ) ) {
					...
					case 'users':
					case 'user':
						$users = get_users( [ 'include' => $current_value ] );
						if ( ! empty( $users ) ) {
				...
				if ( ! empty( $type ) && empty( $data ) ) {
					...
					switch ( $type ) {
						...
						case 'font-icons':
							...
							// phpcs:ignore WordPress.NamingConventions.ValidHookName
							$font_icons = apply_filters( 'redux/font-icons', [] );
