	...
	class Redux_Helpers {
		...
		public static function get_statistics_object() {
			...
			$data = [
				...
				'options'         => apply_filters( 'redux/tracking/options', [] ), // TODO - What is this?!
		 ...
		 * @return array
		 */
		public static function get_request_headers( $args = [] ) {
			$instances = Redux_Instances::get_all_instances();
		...
		private static function get_redux_templates( $custom_template_path ) {
			...
			$template_paths     = [ 'ReduxFramework' => Redux_Core::$dir . 'templates/panel' ];
			$scanned_files      = [];
			$found_files        = [];
		...
		public static function get_developer_keys() {
			...
			// phpcs:ignore WordPress.NamingConventions.ValidHookName
			$data = [ apply_filters( 'redux/tracking/developer', [] ) ];
			if ( 1 === count( $data ) ) {
				if ( empty( $data[0] ) ) {
					$data = [];
				}
			...
			if ( ! file_exists( $path ) || ( file_exists( $path ) && $download && self::google_fonts_update_needed() ) ) {
				if ( $download ) {
					...
					if ( ! is_wp_error( $request ) ) {
						...
						if ( ! empty( $body ) ) {
							$filesystem->execute( 'put_contents', $path, [ 'content' => $body ] );
