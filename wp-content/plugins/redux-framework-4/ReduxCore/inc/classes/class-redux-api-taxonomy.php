...
if ( ! class_exists( 'Redux_Taxonomy' ) ) {
	...
	class Redux_Taxonomy {
		...
		public static function load() {
			add_action( 'init', [ 'Redux_Taxonomy', 'enqueue' ], 99 );
		}
		...
		public static function enqueue() {
			...
			$pagenows = [ 'edit-tags.php', 'term.php', 'admin-ajax.php' ];
		 ...
		 * @return array
		 */
		public static function construct_terms( $opt_name ) {
			$terms = [];
		 ...
		 * @return array
		 */
		public static function construct_sections( $opt_name, $term_id, $permissions = false, $add_visibility = false ) {
			$sections = [];
		 ...
		 * @return array
		 */
		public static function construct_fields( $opt_name = '', $section_id = '', $permissions = false, $add_visibility = false ) {
			$fields = [];
			...
			foreach ( self::$fields[ $opt_name ] as $key => $field ) {
				...
				$field['add_visibility'] = isset( $field['add_visibility'] ) ? $field['add_visibility'] : $add_visibility;
		 ...
		 */
		public static function process_sections_array( $opt_name = '', $term_id = '', $sections = [] ) {
		 ...
		 */
		public static function set_term( $opt_name = '', $term = [] ) {
			self::check_opt_name( $opt_name );
		 ...
		 * @deprecated No longer using camelCase naming convention.
		 */
		public static function setTerm( $opt_name = '', $term = [] ) { // phpcs:ignore WordPress.NamingConventions.ValidFunctionName
			if ( '' !== $opt_name ) {
				Redux_Functions_Ex::record_caller( $opt_name );
			}
		 ...
		 */
		public static function set_terms( $opt_name = '', $terms = [] ) {
		 ...
		 * @return mixed
		 */
		public static function get_terms( $opt_name = '' ) {
			self::check_opt_name( $opt_name );
		 ...
		 */
		public static function get_term_meta( $args = [] ) {
			$default = [
				'key'      => '',
				'opt_name' => '',
				'taxonomy' => '',
