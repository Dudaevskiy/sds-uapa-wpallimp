...
if ( ! class_exists( 'Redux_Extension_Metaboxes_Lite', false ) ) {
	...
	class Redux_Extension_Metaboxes_Lite extends Redux_Extension_Abstract {
			...
			if ( isset( $run_hooks ) && true === $run_hooks ) {
				...
				if ( ! empty( $this->to_replace ) ) {
					foreach ( $this->to_replace as $id => $field ) {
						...
						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						add_filter( "redux/options/{$this->parent->args['opt_name']}/field/{$id}/register", [ $this, 'replace_field' ] );
					}
				}

				add_filter( "redux/options/{$this->parent->args['opt_name']}/options", [ $this, 'override_options' ] );
				add_filter( "redux/field/{$this->parent->args['opt_name']}/_can_output_css", [ $this, 'override_can_output_css' ] );
				add_filter( "redux/field/{$this->parent->args['opt_name']}/output_css", [ $this, 'output_css' ] );
			}
		}
		 ...
		 */
		public function loop_start( $the_post = [] ) {
			if ( is_admin() ) {
				return $the_post;
			}
					...
					foreach ( $box['sections'] as $sk => $section ) {
						...
						if ( isset( $section['fields'] ) ) {
							foreach ( $section['fields'] as $k => $field ) {
								...
								if ( in_array( $field['type'], [ 'ace_editor' ], true ) && isset( $field['options'] ) ) {
		...
		public function add() {
			...
			foreach ( $this->boxes as $key => $box ) {
				...
				if ( isset( $box['position'] ) && ! in_array( strtolower( $box['position'] ), [ 'normal', 'advanced', 'side' ], true ) ) {
					unset( $box['position'] );
				}

				if ( isset( $box['priority'] ) && ! in_array( strtolower( $box['priority'] ), [ 'high', 'core', 'default', 'low' ], true ) ) {
					unset( $box['priority'] );
				}
				...
				if ( isset( $box['post_types'] ) && ! empty( $box['post_types'] ) ) {
					foreach ( $box['post_types'] as $posttype ) {
						...
						add_filter( 'postbox_classes_' . $posttype . '_redux-' . $this->parent->args['opt_name'] . '-metabox-' . $box['id'], [ $this, 'add_box_classes' ] );

						// phpcs:ignore WordPress.NamingConventions.ValidHookName
						do_action( 'redux/' . $this->parent->args['opt_name'] . '/extensions/metabox/add', $this, $box, $posttype );

						// phpcs:ignore Generic.Strings.UnnecessaryStringConcat
						call_user_func( 'add' . '_meta' . '_box', 'redux-' . $this->parent->args['opt_name'] . '-metabox-' . $box['id'], $title, [ $this, 'generate_boxes' ], $posttype, $box['position'], $box['priority'], $args );
					}
				}
			}
		}
		...
		public function meta_boxes_save( $post_id, $post ) {
			...
			if ( ! empty( $this->parent->errors ) || ! empty( $this->parent->warnings ) ) {
				$this->parent->transients['notices'] = ( isset( $this->parent->transients['notices'] ) && is_array( $this->parent->transients['notices'] ) ) ? $this->parent->transients['notices'] : [];
...
if ( ! function_exists( 'redux_metaboxes_loop_start' ) ) {
	 ...
	 */
	function redux_metaboxes_loop_start( $opt_name, $the_post = [] ) {
...
if ( ! function_exists( 'redux_metaboxes_loop_end' ) ) {
	 ...
	 * @param array  $the_post Post object.
	 */
	function redux_metaboxes_loop_end( $opt_name, $the_post = [] ) {
		$redux     = ReduxFrameworkInstances::get_instance( $opt_name );
		$metaboxes = $redux->extensions['metaboxes_lite'];
...
if ( ! function_exists( 'redux_post_meta' ) ) {
	 ...
	 * @return string|void
	 */
	function redux_post_meta( $opt_name = '', $the_post = [], $meta_key = '', $def_val = '' ) {
