...
if ( ! class_exists( 'Redux_Extension_Customizer', false ) ) {
	...
	class Redux_Extension_Customizer extends Redux_Extension_Abstract {
		...
		private function load() {
			...
			add_action( 'customize_register', [ $this, 'register_customizer_controls' ] ); // Create controls.
			...
			add_action( 'wp_enqueue_styles', [ $this, 'custom_css' ], 11 );
		}
					...
					if ( 'heading' !== $option['type'] && 'import_export' !== $option['type'] && ! empty( $option['type'] ) ) {
						...
						$wp_customize->add_setting(
							...
							[
								...
								'transport'         => 'refresh',
								'opt_name'          => $this->parent->args['opt_name'],
								'sanitize_callback' => [ $this, 'field_validation' ],
		...
		public function enqueue_previewer() {
			wp_enqueue_script( 'redux-extension-previewer-js', $this->extension_url . 'assets/js/preview.js', [], self::$version, true );
			...
			foreach ( $this->sections as $section ) {
				if ( isset( $section['fields'] ) ) {
					foreach ( $section['fields'] as $field ) {
						if ( isset( $field['type'] ) ) {
							$field_classes = [ 'Redux_' . $field['type'], 'ReduxFramework_' . $field['type'] ];
